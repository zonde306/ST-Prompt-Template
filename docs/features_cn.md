...
- `[InitialVariables]`：将条目内容视为变量树，写入到初始消息变量内，支持`JSON`与`YAML`格式，且必须是`object`

	> 会先尝试按`JSON`解析，失败后再尝试按`YAML`解析，两者都失败才会报错
	>
	> 只有在启用**立即加载世界书**才会生效
	>
	> 不受**提前加载仅限preload标记**设置影响，始终会被处理
	>
	> 修改将会重新写入，并覆盖之前的内容
...
只处理深度小于此值的楼层，用于避免楼层过多时渲染卡顿

> 深度以最新楼层为`0`开始往前计算
>
> `-1`表示无限制（默认值）
>
> 例如设置为`10`时，只有最新的11个楼层（深度`0`~`10`）会被处理，更早的楼层将保持原样
...
开启后，[立即加载世界书](#立即加载世界书)阶段只处理带有`@@preload`或`@@only_preload`装饰器的条目

> **默认开启**，用于避免世界书条目过多时，打开角色卡卡住
>
> 关闭后将处理所有普通条目（即非`[GENERATE]`、`[RENDER]`、`@INJECT`等特殊条目）
>
> `[InitialVariables]`与`@@initial_variables`不受此设置影响，始终会被处理
>
> 详见[预加载相关装饰器](#预加载相关装饰器)
...
为**世界/知识书**条目启用基于`Monaco`的代码编辑器

开启后，世界书条目的内容框旁边会多出一个**放大**按钮，点击即可打开编辑器

编辑器提供：

- `ejs`语法高亮（包含`<% ... %>`内嵌的`JavaScript`）
- 扩展提供的函数自动补全（`setvar`、`getvar`、`getwi`等）
- 菜单栏可调整**字体**、**显示**、**功能**、**缩进**等选项

> 编辑器设置会保存在浏览器`localStorage`中，下次打开时自动恢复
>
> 点击编辑器的`Save`按钮才会将内容写回世界书条目
...
> 此功能类似于酒馆内置的outlet功能，但不要求绝对顺序，即使`injectPrompt`排在`getPromptsInjected`后面也一样能够正确读取
...
- `@@dont_preload`：不要在打开角色卡时处理这个条目（优先级最高，会覆盖`@@preload`和`@@only_preload`）
- `@@preload`：在[立即加载世界书](#立即加载世界书)阶段必须执行一次（可以与`@@generate_*`、`@@render_*`共存）
- `@@only_preload`：只在[立即加载世界书](#立即加载世界书)阶段启用该条目（生成与渲染阶段都不会激活）
...
- 装饰器必须从条目内容的**第一行**开始，且每个装饰器独占一行，中间不允许有空行或其他内容

- 装饰器可以携带参数，以**第一个空格**分割装饰器名与参数

	> 例如`@@if variables.好感度 > 50`中，`@@if`为装饰器名，`variables.好感度 > 50`为参数
	>
	> 又例如`@@iframe 折叠状态栏（点击显示）`中，`折叠状态栏（点击显示）`为参数

- 无法识别的`@@xxx`行会被**丢弃**（既不会作为装饰器生效，也不会保留在条目内容里）

	> 只有上面**可用装饰器列表**中的名称才会被识别
	>
	> 因此装饰器写错名字时，这一行会直接消失

- 使用`@@@`前缀可以转义，避免开头的内容被当作装饰器处理

	> 例如条目内容以`@@@activate`开头时，`@@activate`不会生效，该行会作为普通文本保留

### 预加载相关装饰器

**立即加载世界书**（预加载）阶段是否处理某个条目，按以下顺序判断：

1. 带有`@@dont_preload`：**不处理**
2. 带有`@@preload`或`@@only_preload`：**处理**
3. 启用了**提前加载仅限preload标记**设置（默认启用）：**不处理**
4. 否则：只处理非特殊条目（即不带`[GENERATE:*]`、`[RENDER:*]`、`@INJECT`、`@@only_preload`等标记的普通条目）

> 因为默认启用**提前加载仅限preload标记**，所以想要在打开角色卡时执行的条目（例如初始化变量、`define`），需要显式添加`@@preload`或`@@only_preload`
>
> `[InitialVariables]`与`@@initial_variables`不受此限制，始终会在预加载阶段处理
